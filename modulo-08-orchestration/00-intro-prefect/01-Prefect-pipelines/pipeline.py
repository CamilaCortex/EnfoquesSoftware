...
from src.data import read_dataframe, save_dataframe_to_csv, validate_data, calculate_next_period
...
def duration_prediction_flow(year: int = None, month: int = None) -> str:
    ...
    # Load training data
    df_train = read_dataframe(year=year, month=month, dataset_type="train")
    
    # Save training data to CSV (always executes, no cache)
    save_dataframe_to_csv(df_train, year=year, month=month, dataset_type="train")
    ...
    df_val = read_dataframe(year=next_year, month=next_month, dataset_type="val")
    
    # Save validation data to CSV (always executes, no cache)
    save_dataframe_to_csv(df_val, year=next_year, month=next_month, dataset_type="val")
